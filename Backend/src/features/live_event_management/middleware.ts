...
import type { AdvanceRoundRequestBody, DeclareWinnersRequestBody } from "./types.js";
...
export async function validateDeclareWinners(c: Context, next: Next) {
    const id = c.req.param("id")
    const parsedId = Number(id)
    if (!Number.isInteger(parsedId) || parsedId <= 0) {
        throw new AppError("ROUND_ID_INVALID", { field: "declare_winners_input_id" })
    }

    let body: DeclareWinnersRequestBody = {}
    try {
        body = await c.req.json<DeclareWinnersRequestBody>()
    } catch {
        body = {}
    }
    ...
    if (selectedContestantIds !== undefined) {
        ...
        parsedSelectedIds = selectedContestantIds.map((contestantId, index) => {
            ...
            if (!Number.isInteger(parsed) || parsed <= 0) {
                throw new AppError("SELECTED_CONTESTANT_ID_INVALID", {
                    field: `declare_winners_input_selected_contestant_ids_${index}`,
                })
            }
            return parsed
        })
