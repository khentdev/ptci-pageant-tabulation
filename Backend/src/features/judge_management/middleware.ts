...
export async function validateAddJudgeInput(c: Context, next: Next) {
    const { name, username, password, role } = await c.req.json<AddJudgeInputRequestBody>()
    ...
    let parsedRole: "JUDGE" | "CHAIRMAN" = "JUDGE"
    if (role !== undefined) {
        if (role !== "JUDGE" && role !== "CHAIRMAN") {
            throw new AppError("JUDGE_ROLE_INVALID", { field: "judge_role_input" })
        }
        parsedRole = role
    }
