...
Permanently deletes a round. Allowed only when the round has no categories and no contestants have advanced into it. The frontend should always show the delete action; if the round is locked, the backend rejects the request and the UI shows an error toast.
