...
Permanently deletes a contestant. Allowed only when no judge scores exist for that contestant. The frontend should always show the delete action; if the contestant is locked, the backend rejects the request and the UI shows an error toast.
...
| `400` | `CONTESTANT_LOCKED` | Contestant cannot be edited because it has scores already. | Rejected when any scores exist for the contestant |
| `404` | `CONTESTANT_NOT_FOUND` | Contestant not found. | Includes deleting the same contestant twice |
