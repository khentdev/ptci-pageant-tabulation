...
**Scope note:** This endpoint returns judge-submission data only. Rankings, `canAdvance`, `advancement`, `nextRound`, and related flags are served by [[live-event/live-round-results]] (`GET /live-event/round-results/:id/advancement`). Advance write is [[live-event/live-round-advance]]. Fetch judge submissions and round results on page mount / refresh / round change.
