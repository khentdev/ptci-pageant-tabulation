    ...
    <div class="flex w-full max-w-sm flex-col items-center gap-6 rounded-xl bg-white/95 p-6 text-center shadow-lg">
      ...
      <div class="flex flex-col gap-2">
        ...
        <p id="not-found-description" class="text-sm text-custom-black/60">
          The round you're looking for doesn't exist or may have been removed.
        </p>
      </div>

      <router-link
        v-if="firstRoundId"
        :to="{ name: 'live-event-results', params: { roundId: firstRoundId } }"
...
const firstRoundId = computed(() => roundStore.roundList[0]?.id ?? null);
</script>
