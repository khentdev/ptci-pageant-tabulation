...
<template>
  ...
  >
    <div
      class="group flex items-center justify-center overflow-hidden rounded-t-2xl border-b border-black/15 bg-black/10 transition-all"
    >
      <img
    ...
    <div class="my-8 flex w-full items-center px-4">
      <div class="flex flex-col gap-4">
        <p class="text-3xl font-semibold italic">{{ can.canNumber }}</p>
        <p class="text-xl font-medium">{{ can.canName }}</p>
        <div class="flex w-full items-center gap-2">
          <p class="bg-main-dark-brown rounded-2xl border border-black/15 px-4 py-1 text-white/90">
            {{ can.canTeamName }}
          </p>
          <p class="bg-main-dark-brown rounded-2xl border border-black/15 px-4 py-1 text-white/90">
            {{ can.canCourse }}
